...
pub fn extract_toc(markdown: &str) -> Vec<TocItem> {
    ...
    extract_headings(root, &mut toc_items, &mut anchorizer);
    ...
    match &node.data.borrow().value {
        ...
        _ => {
            ...
            for child in node.children() {
                extract_headings(child, toc_items, anchorizer);
            }
        }
    }
}
...
mod tests {
    use super::*;
    use crate::md::parser::markdown_to_html;
    ...
    #[test]
    fn test_extract_toc_ids_match_rendered_heading_ids() {
        let markdown = "# Test & Demo\n## Test & Demo";
        let toc = extract_toc(markdown);
        let html = markdown_to_html(markdown);

        assert_eq!(toc.len(), 2);
        assert_eq!(toc[0].id, "test--demo");
        assert_eq!(toc[1].id, "test--demo-1");
        assert!(html.contains("id=\"test--demo\""));
        assert!(html.contains("id=\"test--demo-1\""));
    }
